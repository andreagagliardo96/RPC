...
{
  ros::init(argc, argv, "move_group_interface_hw");
  ros::NodeHandle node_handle;

  ros::AsyncSpinner spinner(1);
  spinner.start(); //needed to get info about the robot' state (before MoveGroupInteface)



  static const std::string PLANNING_GROUP = "panda_arm"; //name of the planning group you would like to control and plan for
  ...
  // RViz provides many types of markers; we will use text, cylinders, and spheres
  Eigen::Isometry3d text_pose = Eigen::Isometry3d::Identity();
  text_pose.translation().z() = 1.0;
  visual_tools.publishText(text_pose, "MoveGroupInterface homework", rvt::RED, rvt::XLARGE);

  // Batch publishing is used to reduce the number of messages being sent to RViz for large visualizations
  visual_tools.trigger();

  // We can print the name of the reference frame for this robot.
  ROS_INFO_NAMED("homework", "Planning frame: %s", move_group_interface.getPlanningFrame().c_str());

  // We can also print the name of the end-effector link for this group.
  ROS_INFO_NAMED("homework", "End effector link: %s", move_group_interface.getEndEffectorLink().c_str());
  
  // We can get a list of all the groups in the robot:
  ROS_INFO_NAMED("homework", "Available Planning Groups:");
  std::copy(move_group_interface.getJointModelGroupNames().begin(),
            move_group_interface.getJointModelGroupNames().end(), std::ostream_iterator<std::string>(std::cout, ", "));

  // Start the demo
  // ^^^^^^^^^^^^^^^^^^^^^^^^^
  visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window to plan the trajectory to a target position");
  ...
  ROS_INFO_NAMED("homework", "Visualizing plan 2 (joint space goal) %s", success ? "" : "FAILED");
  ...
  visual_tools.publishTrajectoryLine(my_plan.trajectory_, joint_model_group);
  visual_tools.trigger();
  visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window to plan with constraint");
  ...
  success = (move_group_interface.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
  ROS_INFO_NAMED("homework", "Visualizing plan 3 (constraints) %s", success ? "" : "FAILED");
  ...
  target_pose3.position.z  = 0.3; //-= 0.2;
  waypoints.push_back(target_pose3);  // down

  target_pose3.position.y -= 0.2;
  waypoints.push_back(target_pose3);  // right

  target_pose3.position.z += 0;//0.2;
  target_pose3.position.y += 0.2;
  ...
  ROS_INFO_NAMED("homework", "Visualizing plan 4 (Cartesian path) (%.2f%% acheived)", fraction * 100.0);
  ...
  visual_tools.trigger();
  visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window to plan trajectory without obstacle");
  ...
  success = (move_group_interface.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
  ROS_INFO_NAMED("homework", "Visualizing plan 5 (with no obstacles) %s", success ? "" : "FAILED");
  ...
  ROS_INFO_NAMED("homework", "Add an object into the world");
  ...
  success = (move_group_interface.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
  ROS_INFO_NAMED("homework", "Visualizing plan 6 (pose goal move around cuboid) %s", success ? "" : "FAILED");
  ...
  cylinder_primitive.dimensions[primitive.CYLINDER_HEIGHT] = 0.3;//0.20;
  cylinder_primitive.dimensions[primitive.CYLINDER_RADIUS] = 0.1;//0.04;
